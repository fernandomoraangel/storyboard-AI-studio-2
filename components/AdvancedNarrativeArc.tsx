...
export const AdvancedNarrativeArc: React.FC<AdvancedNarrativeArcProps> = ({
  projectState,
  onUpdate,
}) => {
  const { t } = useLanguage();
  ...
  const [activePoint, setActivePoint] = useState<{
    index: number;
    type: "tension" | "emotion" | "conflict";
  } | null>(null);
  const keysPressed = useRef<{ n: boolean; d: boolean; shift: boolean }>({
    n: false,
    d: false,
    shift: false,
  });
  ...
  const [selectedCurve, setSelectedCurve] = useState<
    "tension" | "emotion" | "conflict"
  >("tension");
  const [visibleCurves, setVisibleCurves] = useState<
    Set<"tension" | "emotion" | "conflict">
  >(new Set(["tension"]));
  ...
  const seriesArc = useMemo(() => {
    const sorted = (projectState.narrativeArc || []).sort(
      (a, b) => (a.x || 0) - (b.x || 0)
    );
    ...
    if (
      ...
      (withBoundaries[withBoundaries.length - 1].x || 0) < 100
    ) {
      withBoundaries.push({
        id: -2,
        label: "End",
  ...
  const episodeArc = useMemo(() => {
    ...
    const index = projectState.episodes.findIndex(
    ...
    const insideIndices = mappedAll
      .map((p, i) => (p.x! >= 0 && p.x! <= 100 ? i : -1))
      .filter((i) => i !== -1);
    ...
    return mappedAll
      .filter((_, i) => indicesToKeep.has(i))
  ...
  const sceneArc = useMemo(() => {
    ...
    const totalEpisodes = projectState.episodes.length;
    const epIndex = projectState.episodes.findIndex(
      (e) => e.id === currentEpisode.id
    );

    const totalScenes = currentEpisode.scenes.length;
    const sceneIndex = currentEpisode.scenes.findIndex(
  ...
  const activeShots = useMemo(() => {
    ...
    if (scope === "episode" && currentEpisode)
      return currentEpisode.scenes.flatMap((s) => s.shots);
    return projectState.episodes.flatMap((e) =>
  ...
  const shotPositions = useMemo(() => {
    ...
    if (scope === "series") {
      ...
      projectState.episodes.forEach((ep, epIndex) => {
        ...
        const totalShots = ep.scenes.reduce(
          (acc, s) => acc + s.shots.length,
          0
        );
  ...
  const generatePath = (curve: "tension" | "emotion" | "conflict") => {
    if (activeArc.length === 0) return "";

    const points = activeArc.map((p) =>
      getCoordinates(p.x || 0, p[curve] || 0)
    );
  ...
  const arcPaths = useMemo(() => {
    const paths: Array<{
      curve: "tension" | "emotion" | "conflict";
      path: string;
      color: string;
    }> = [];
    ...
    return paths;
  }, [activeArc, curveType, visibleCurves]);

  // --- Interaction Handlers ---
  const handleMouseDown = (e: React.MouseEvent) => {
    if (!svgRef.current) return;
    const { x, value } = getValueFromCoordinates(
    ...
    if (keysPressed.current.shift && keysPressed.current.n) {
      ...
      const newPoints = [...activeArc, newPoint].sort(
        (a, b) => (a.x || 0) - (b.x || 0)
      );
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDragging && activePoint) {
      ...
      const targetShotIndex = shotPositions.findIndex(
        (p) => progress >= p.start && progress < p.end
      );
      if (targetShotIndex !== -1 && filmstripRef.current) {
        const shotElement = filmstripRef.current.children[
          targetShotIndex
        ] as HTMLElement;
        if (shotElement) {
          const containerCenter = filmstripRef.current.clientWidth / 2;
          const shotCenter =
            shotElement.offsetLeft + shotElement.clientWidth / 2;
  ...
  const handleFilmstripClick = (index: number) => {
    ...
    const progress =
      (index / activeShots.length) * 100 + 100 / activeShots.length / 2; // Center of shot
    setCursorProgress(progress);
  };
    ...
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (
                    projectState.episodes.find((ep) => ep.id === id)?.scenes
                      .length
                  ) {
                    setSelectedSceneId(
                      projectState.episodes.find((ep) => ep.id === id)!
                        .scenes[0].id
                    );
                  }
                }}
                className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 outline-none font-medium"
              >
                <option value="" disabled>
                  Select Episode
                </option>
                {projectState.episodes.map((ep, index) => (
          ...
          {scope === "scene" && currentEpisode && (
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {currentEpisode.scenes.map((s, index) => (
            ...
            {(["tension", "emotion", "conflict"] as const).map((c) => (
              <button
                ...
                onClick={() => {
                  ...
                  if (!newVisible.has(selectedCurve)) {
                    setSelectedCurve(Array.from(newVisible)[0]);
                  }
                }}
      ...
      <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
        <div className="flex items-center gap-2 mb-3 text-gray-400 text-sm">
          ...
          <span>
            Narrative Arc ({scope.charAt(0).toUpperCase() + scope.slice(1)}{" "}
            Scope)
          </span>
        </div>
          ...
          >
            <defs>
              ...
              >
                <stop
                  offset="0%"
                  stopColor={
                    selectedCurve === "tension"
                      ? "#ef4444"
                      : selectedCurve === "emotion"
                      ? "#06b6d4"
                      : "#f59e0b"
                  }
                />
            ...
            {arcPaths.map(({ curve, path, color }) => (
              <path
                ...
                opacity={visibleCurves.has(curve) ? 0.8 : 0.2}
              />
            ))}

            {/* Arc Points for all visible curves */}
            {Array.from(visibleCurves).map((curve) => (
              <g key={`points-${curve}`}>
                {activeArc.map((point, index) => {
                  ...
                  const isSelected =
                    selectedPointIndex === index && selectedCurve === curve;
                  const color =
                    curve === "tension"
                      ? "#ef4444"
                      : curve === "emotion"
        ...
        >
          {activeShots.length > 0 ? (
            activeShots.map((shot, idx) => {
              ...
              const isActive =
                pos && cursorProgress >= pos.start && cursorProgress < pos.end;

              return (
                <div
                  key={shot.id}
                  onClick={() => handleFilmstripClick(idx)}
                  className={`flex-shrink-0 w-40 aspect-video bg-gray-900 rounded-lg overflow-hidden border-2 cursor-pointer transition-all ${
                    isActive
