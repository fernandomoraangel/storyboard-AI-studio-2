  ...
  const filteredData = useMemo(() => {
    ...
    if (filterText) {
      ...
      resultEpisodes = resultEpisodes
        ...
        .filter(
          (ep) =>
            ep.title.toLowerCase().includes(lowerFilter) || ep.scenes.length > 0
        );
    }
        ...
        .map((ep) => ({
          ...
          scenes: ep.scenes
            .map((scene) => ({
              ...scene,
              shots: scene.shots.filter(
                (shot) => shot.subplot === selectedSubplot
              ),
            }))
    ...
    if (selectedEpisodeId) {
      resultEpisodes = resultEpisodes.filter(
    ...
    const allShots = filteredScenes.flatMap((scene) =>
      scene.shots.map((shot) => ({
        ...
        scene,
        episode:
          resultEpisodes.find((e) => e.id === scene.episodeId) ||
          resultEpisodes[0], // Fallback safe
      }))
    );
      ...
      {filteredData.scenes.map((scene) => (
        ...
        >
          ...
          <div className="p-3">
            ...
            <h3
              className="font-medium text-white text-sm mb-1 truncate"
              title={scene.title}
            >
              {scene.title}
            </h3>
  ...
  const renderShots = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 p-6">
      {filteredData.shots.map((item, idx) => {
        // Calculate shot number within the scene
        const shotIndex =
          item.scene.shots.findIndex((s) => s.id === item.shot.id) + 1;

        // Calculate scene consecutive number within the episode
        const sceneConsecutive =
          item.episode.scenes.findIndex((s) => s.id === item.scene.id) + 1;

        // Get shot type abbreviation (first 2-3 letters)
        const shotTypeAbbr = item.shot.shotType.substring(0, 3).toUpperCase();
        ...
        return (
          ...
          >
            <div className="aspect-video bg-gray-900 relative overflow-hidden">
              ...
              {/* Scene-Shot number in top-left */}
              <div className="absolute top-1 left-1 bg-black/60 px-1.5 py-0.5 rounded text-[10px] text-white font-semibold">
                {sceneConsecutive}-{shotIndex}
              </div>
              {/* Shot type abbreviation and duration in bottom-right */}
              <div className="absolute bottom-1 right-1 bg-black/60 px-1.5 py-0.5 rounded text-[10px] text-white font-medium">
                {shotTypeAbbr} {item.shot.duration}s
              </div>
            </div>
            <div className="p-2 flex-1 flex flex-col">
              <p
                className="text-gray-300 text-xs line-clamp-2 mb-auto"
                title={item.shot.description}
              >
              ...
              <div className="mt-2 pt-2 border-t border-gray-700 flex justify-between items-center">
                ...
                {item.shot.subplot && (
                  <span
                    ...
                    title={`Subplot: ${item.shot.subplot}`}
                  >
              ...
              {selectedSceneId && (
                <>
                  ...
                  <span className="text-white">
                    Scene{" "}
                    {
                      filteredData.scenes.find((s) => s.id === selectedSceneId)
                        ?.title
                    }
                  </span>
                </>
            ...
            {(["episodes", "scenes", "shots"] as ViewMode[]).map((mode) => (
              <button
                ...
                onClick={() => {
                  setViewMode(mode);
                  setSelectedSceneId(null);
                  if (mode === "episodes") setSelectedEpisodeId(null);
                }}
