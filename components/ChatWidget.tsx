...
import type { ChatMessage, Scene, Character, StoryboardStyle, CreativeProfile } from '../types';
...
export const ChatWidget: React.FC<ChatWidgetProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      setIsLoading(true);
      try {
        chatRef.current = await createChat(props.characters, props.scenes, language, props.activeProfile);
      } catch (error) {
  ...
  const handleToolCalls = async (calls: FunctionCall[]): Promise<Part[]> => {
    ...
    await Promise.all(calls.map(async (call) => {
      try {
        ...
        toolResponses.push({ functionResponse: { name: call.name, response: result } } as any);

      } catch (e) {
        console.error(`Error executing tool ${call.name}:`, e);
        toolResponses.push({ functionResponse: { name: call.name, response: { error: (e as Error).message } } } as any);
      }
    }));
