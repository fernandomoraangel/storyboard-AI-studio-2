...
export const Home: React.FC<HomeProps> = ({ setWorkflowPhase }) => {
    const { t } = useLanguage();
    ...
    const flowNodes = [
        {
            ...
            label: 'Setup',
            nodes: [
                { id: 'creativeTeam', icon: CameraIcon, labelKey: 'creativeTeam', descKey: 'creativeTeamDesc' },
                { id: 'authors', icon: UsersIcon, labelKey: 'authorsTitle', descKey: 'authorsDesc' }
            ]
        },
        {
            id: 'ideation',
            label: 'Ideation',
            nodes: [
                { id: 'generator', icon: LayoutGridIcon, labelKey: 'storyGeneratorTab', descKey: 'storyGeneratorDesc' }
            ]
        },
        {
            id: 'structure',
            label: 'Structure',
            nodes: [
                { id: 'arc', icon: ActivityIcon, labelKey: 'narrativeArcTitle', descKey: 'narrativeArcDesc' },
                { id: 'bible', icon: BookOpenIcon, labelKey: 'outlineTab', descKey: 'seriesBibleDesc' }
            ]
        },
        {
            id: 'refinement',
            label: 'Refinement',
            nodes: [
                { id: 'episodes', icon: FolderOpenIcon, labelKey: 'episodes', descKey: 'episodesDesc' },
                { id: 'organizer', icon: LayoutGridIcon, labelKey: 'visualOrganizerTitle', descKey: 'visualOrganizerDesc' }
            ]
        },
        {
            id: 'visualization',
            label: 'Visualization',
            nodes: [
                { id: 'storyboard', icon: FilmIcon, labelKey: 'storyBoardTab', descKey: 'storyboardDesc' },
                { id: 'gallery', icon: GalleryIcon, labelKey: 'galleryView', descKey: 'galleryViewDesc' }
            ]
        },
        {
            id: 'production',
            label: 'Production',
            nodes: [
                { id: 'video', icon: VideoIcon, labelKey: 'videoGeneratorTab', descKey: 'videoGeneratorDesc' }
            ]
        },
        {
            id: 'tools',
            label: 'Tools',
            nodes: [
                { id: 'utilities', icon: ChartBarIcon, labelKey: 'utilitiesTitle', descKey: 'utilitiesDesc' }
            ]
        }
    ];
            ...
            <div className="relative w-full max-w-5xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                    {flowNodes.map((group, index) => (
                        <div key={group.id} className="flex flex-col items-center gap-4 relative group">
                            {/* Group Label */}
                            ...
                            <div className="flex flex-col gap-3 w-full">
                                {group.nodes.map((node) => (
                                    ...
                                    >
                                        <button
                                            onClick={() => setWorkflowPhase(node.id)}
                                        ...
                                        {hoveredNode === node.id && (
                                            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 w-64 bg-gray-900 border border-indigo-500/50 rounded-lg p-3 shadow-2xl z-50 animate-fade-in">
                                                ...
                                                <div className="text-xs text-gray-400 leading-relaxed">
                                                    {t(node.descKey as any) || "Description not available."}
                                                </div>
                                                {/* Arrow */}
