...
const SelectField: React.FC<{ label: string; value: string; onChange: (e: ChangeEvent<HTMLSelectElement>) => void; name: string; options: readonly string[]; unassignedLabel: string }> = ({ label, options, unassignedLabel, ...props }) => (
  <div>
    <label className="block text-sm font-medium text-gray-400 mb-1">{label}</label>
    <select
      className="block w-full rounded-md border-0 bg-white/5 py-1.5 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6"
...
export const ShotCard: React.FC<ShotCardProps> = ({ shot, shotNumber, scene, characters, updateShot, deleteShot, isLastShot, storyboardStyle, aspectRatio, isDragging, onDragStart, onDragEnd }) => {
  ...
  const getCharacterDetailsForPrompt = useCallback((): string => {
    ...
    const sceneCharacterNames = scene.characters.split(',').map(name => name.trim().toLowerCase());
    const includedCharacters = characters.filter(char => sceneCharacterNames.includes(char.name.toLowerCase()));
    
    if (includedCharacters.length === 0) return `Characters: ${scene.characters}.`;

    return 'Characters involved: ' + includedCharacters.map(char => {
  ...
  const getEnglishPromptDetails = useCallback(async (): Promise<any> => {
    ...
    if (language === 'en') {
      return { ...shot, setting: scene.setting, characterDetails: characterDetailsString };
    }
    ...
    const translatedTextFields = await translateDetailsToEnglish(textFieldsToTranslate);

    const findEnglishOption = (spanishValue: string, optionKey: keyof Options): string => {
    ...
    return {
      ...
      cameraMovement: findEnglishOption(shot.cameraMovement, 'cameraMovementOptions'),
  ...
  return (
    <div className={`bg-gray-900/40 rounded-lg border border-gray-700 ${isDragging ? 'dragging-item' : ''}`} onDragOver={(e) => e.preventDefault()}>
      <div 
      ...
      >
        ...
        <div className="flex items-center gap-4 flex-1 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
          <h4 className="text-md font-bold text-indigo-300">{t('shot')} {shotNumber}</h4>
          <div className="flex items-center gap-2">
            <label htmlFor={`duration-${shot.id}`} className="text-xs font-medium text-gray-400">{t('duration')}</label>
            <input
              id={`duration-${shot.id}`}
              type="number"
              name="duration"
      ...
      >
        <div className="px-4 pb-4 space-y-4">
          ...
          <TextAreaField label={t('description')} name="description" value={shot.description} onChange={handleChange} rows={4} />
          <InputField label={t('soundFx')} name="soundFx" value={shot.soundFx} onChange={handleChange} />
          
          <div className="border-t border-gray-700 pt-4">
            <h4 className="text-base font-semibold text-gray-300 mb-4">{t('technicalDetails')}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <SelectField label={t('shotType')} name="shotType" value={shot.shotType} onChange={handleChange} options={options.shotTypeOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('cameraMovement')} name="cameraMovement" value={shot.cameraMovement} onChange={handleChange} options={options.cameraMovementOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('cameraType')} name="cameraType" value={shot.cameraType} onChange={handleChange} options={options.cameraTypeOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('lensType')} name="lensType" value={shot.lensType} onChange={handleChange} options={options.lensTypeOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('lensBlur')} name="lensBlur" value={shot.lensBlur} onChange={handleChange} options={options.lensBlurOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('lightingScheme')} name="lighting" value={shot.lighting} onChange={handleChange} options={options.lightingOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('style')} name="style" value={shot.style} onChange={handleChange} options={options.styleOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('colorGrade')} name="colorGrade" value={shot.colorGrade} onChange={handleChange} options={options.colorGradeOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('filmStock')} name="filmStock" value={shot.filmStock} onChange={handleChange} options={options.filmStockOptions} unassignedLabel={t('unassigned')} />
                <SelectField label={t('filmGrain')} name="filmGrain" value={shot.filmGrain} onChange={handleChange} options={options.filmGrainOptions} unassignedLabel={t('unassigned')} />
                <div className="sm:col-span-2">
                    <InputField label={t('atmosphere')} name="atmosphere" value={shot.atmosphere} onChange={handleChange} />
                </div>
                <div className="sm:col-span-2">
                    <TextAreaField label={t('otherNotes')} name="technicalNotes" value={shot.technicalNotes} onChange={handleChange} rows={2}/>
                </div>
                 <div className="sm:col-span-2">
                    <TextAreaField label={t('shotNotes')} name="notes" value={shot.notes} onChange={handleChange} rows={2} placeholder={t('shotNotesPlaceholder') as string}/>
                </div>
            </div>
          </div>
          
          <div className="border-t border-gray-700 pt-4">
              <h4 className="text-base font-semibold text-gray-300 mb-4">{t('promptGenerators')}</h4>
              <div className="space-y-3">
                  ...
                  {generatedImagePrompt && (
                      <div className="relative">
                          <textarea readOnly value={generatedImagePrompt} className="block w-full rounded-md border-0 bg-white/5 py-1.5 text-gray-300 shadow-sm ring-1 ring-inset ring-white/10 sm:text-sm" rows={4}/>
                          <button onClick={copyImagePrompt} className="absolute top-2 right-2 p-1.5 rounded-md bg-gray-900/50 hover:bg-gray-700 text-blue-400 hover:text-blue-300">
                             <ClipboardIcon className="w-4 h-4" />
                          </button>
                          {isImagePromptCopied && <span className="absolute top-2 right-10 text-xs text-indigo-400 bg-gray-900 px-2 py-1 rounded">{t('copied')}</span>}
                      </div>
                  )}
              </div>
          </div>
